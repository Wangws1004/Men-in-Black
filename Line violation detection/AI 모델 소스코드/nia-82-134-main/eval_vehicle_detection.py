...
from utils import merge_vehicle_annotation, setup_logger
...
parser.add_argument(
    ...
    default="./configs/vehicle_detection_config.py",
)
parser.add_argument(
    "--launcher",
...
parser.add_argument("--save_dir", default="./eval_logs/vehicle_detection")
...
def get_test_result(cfg):
    ...
    if not distributed:
        ...
        outputs = multi_gpu_test(model, data_loader, args.save_dir, True)
...
def main():
    logger.info("Start running eval_vehicle_detection.py")
    ...
    if not args.no_merging:
        for split in ["test"]:
            ...
            merge_vehicle_annotation(annot_list, save_path)
    cfg.data.test.test_mode = True
    ...
    result_df.to_csv(
        os.path.join(args.save_dir, "eval_vehicle_merged_df.csv"),
        encoding="utf-8",
        index_label="No.",
    )
